...
class FPCA_penalized_arbPenal:
    def __init__(self, moneyness, tau, implVol, nb_spline_moneyness = 10, nb_spline_tau = 10, order_moneyness = 4, order_tau = 4, S=None, r=None, q=None, range_moneyness = [-.15, .15], range_tau = [0, 1], dailyWeights = None):
        ...
        if dailyWeights is None:
            self.dailyWeights = np.repeat(1, len(implVol))
        elif len(dailyWeights) != len(implVol):
            raise ValueError("the length of the daily weight vector has to be the same as the number of days observed len(implVol)")
        else:
            self.dailyWeights = dailyWeights
        
        # Clean data once to avoid repeated computation in iterations
        self.cleaned_data = []
        for i in range(len(implVol)):
            m_f = np.asarray(moneyness[i]).flatten()
            t_f = np.asarray(tau[i]).flatten()
            implVol_f = np.asarray(implVol[i]).flatten()
            mask = ~np.isnan(implVol_f) & ~np.isnan(m_f) & ~np.isnan(t_f)
            self.cleaned_data.append((m_f[mask], t_f[mask], implVol_f[mask]))
    ...
    def first_FPC_fit(self, threshold = 1e-4, maxit = 10, omega_m = 1.0, omega_m2 = 0.0, omega_t = 1.0, d_m = 2, d_m2 = 3, d_t = 2, monotone = True):
        ...
        while j < maxit:
            ...
            for i in range(len(self.cleaned_data)):
                ...
                reg.fit(X, implVol_clean)

                alpha_i = reg.coef_[0]
                scores.append(alpha_i)
                mse_list.append(np.mean((implVol_clean - reg.predict(X))**2))
            ...
            for i in range(len(self.cleaned_data)):
                m_clean, tau_clean, implVol_clean = self.cleaned_data[i]
                if len(implVol_clean) == 0: continue
                ...
                X_i = np.einsum('ja,jb->jab', D_m, D_t).reshape(len(implVol_clean), -1)
    ...
    def _build_monotone_operator(self):
        """
        Constructs the discrete maturity difference operator P_1 for total volatility monotonicity.
        P_1 @ vec(B) approximates the maturity slope of total volatility for an eigenfunction.
        """
        S_m = self.nb_spline_moneyness
        S_tau = self.nb_spline_tau
        ...
        np.fill_diagonal(D_diff_t_monotone, np.concatenate(([0, 0], -(np.sqrt(self.t_t[4:-3]) / np.sqrt(self.t_t[5:-2])))))
    ...
    def _compute_calendar_envelope_bounds(self, P_1, q_lower=99, q_upper=1):
        ...
        ensuring composite total volatility remains non-decreasing across sample days.
    ...
    def reconstruct_surface(self, coords, BList, scores):
        """
        Reconstructs the implied volatility surface at given coordinates using provided Bs and scores.
        ...
        Returns:
        - reconstructed_implVol: 1D array of length n
        """
        reconstructed_implVol = np.zeros(len(coords))
        for k in range(len(BList)):
            reconstructed_implVol += scores[k] * self.evaluator(BList[k], coords)
        return reconstructed_implVol

    def project_to_scores(self, coords, implVol, BList):
        """
        Projects the given observations (coords and implVol) onto the list of eigenfunctions (BList)
        to estimate the scores using linear regression without intercept.
        
        Parameters:
        - coords: 2D array of shape (n, 2)
        - implVol: 1D array of implied volatilities corresponding to coords
        - BList: List of 2D B matrices representing the eigenfunctions
        
        Returns:
        - scores: 1D array of length len(BList)
        """
        if len(implVol) == 0 or len(BList) == 0:
            return np.zeros(len(BList))
            
        X = np.zeros((len(implVol), len(BList)))
        for k in range(len(BList)):
    ...
    def _evaluate_fold_day(self, train_idx, val_idx, omega_m, omega_m2, omega_t, fpc_index, previous_BList,
        ...
        """
        # Create a new FPCA_penalized_arbPenal instance for training on the subset of days
        fpca_train = FPCA_penalized_arbPenal(
            moneyness=[], tau=[], implVol=[],
        ...
        if fpc_index == 0:
            ...
            for local_i, global_i in enumerate(train_idx):
                m_clean, tau_clean, implVol_clean = self.cleaned_data[global_i]
                if len(implVol_clean) > 0:
                    coords = np.column_stack([m_clean, tau_clean])
                    fpca_train.scoreMat[local_i, :] = fpca_train.project_to_scores(coords, implVol_clean, previous_BList)
        ...
        val_but_viols = []
        for i in val_idx:
            m_val, tau_val, implVol_val = self.cleaned_data[i]
            if len(implVol_val) == 0:
                continue
            coords_val = np.column_stack([m_val, tau_val])
            scores_val = self.project_to_scores(coords_val, implVol_val, BList_val)
            y_hat_val = self.reconstruct_surface(coords_val, BList_val, scores_val)
            mse_val = np.mean((implVol_val - y_hat_val)**2)
    ...
    def _evaluate_fold_observation(self, fold_obs_splits, fold_idx, omega_m, omega_m2, omega_t, fpc_index, previous_BList,
        ...
        for i in range(n_days):
            ...
            if len(train_idx) > 0:
                train_cleaned_data.append((m_clean[train_idx], tau_clean[train_idx], implVol_clean[train_idx]))
            else:
                train_cleaned_data.append((np.array([]), np.array([]), np.array([])))
                
            if len(val_idx) > 0:
                val_data.append((np.column_stack([m_clean[val_idx], tau_clean[val_idx]]), implVol_clean[val_idx], train_idx))
            else:
        ...
        if fpc_index == 0:
            ...
            for i in range(n_days):
                m_clean, tau_clean, implVol_clean = train_cleaned_data[i]
                if len(implVol_clean) > 0:
                    coords = np.column_stack([m_clean, tau_clean])
                    fpca_train.scoreMat[i, :] = fpca_train.project_to_scores(coords, implVol_clean, previous_BList)
            _, B_new = fpca_train.subsequent_FPC_fit(
                threshold=threshold, maxit=maxit, omega_m=omega_m, omega_m2=omega_m2, omega_t=omega_t, d_m=d_m, d_m2=d_m2, d_t=d_t
            )
        ...
        for i in range(n_days):
            ...
            if val_info is None:
                continue
            coords_val, implVol_val, train_idx = val_info
            
            m_clean, tau_clean, implVol_clean = self.cleaned_data[i]
            coords_train = np.column_stack([m_clean[train_idx], tau_clean[train_idx]])
            implVol_train = implVol_clean[train_idx]
            
            if len(implVol_train) == 0:
                continue
                
            scores_train_i = self.project_to_scores(coords_train, implVol_train, BList_val)
    ...
    def implVol_to_price_surface(self, coords, implVol, day_index=0):
        """
        ...
        iv_safe = np.maximum(implVol, 1e-10)
    ...
    def BSplines_2d(self, range_moneyness=[-.15, .1], range_tau=[0, 1],
        ...
        def evaluator(B, coords):
            ...
            Returns:
            - 1D array of length n containing the evaluated implied volatilities.
            """
    ...
    def plot_reconstruction(self, index, num_points=30, figAngle=-70):
        ...
        - figAngle: Viewing angle (azimuth).
        """
        m_clean, tau_clean, implVol_clean = self.cleaned_data[index]
        ...
        # Reconstruct on grid (returns implied volatility sigma)
        ...
        ax.scatter(m_clean, tau_clean, implVol_clean, color='red', s=10, label='Actual Observations')
    ...
    def compute_arbitrage_metrics(self, scores, m_grid=None, tau_grid=None):
        """
        Computes calendar and butterfly arbitrage metrics on a grid using total variance derived from implied volatility.
        ...
        # implVol is sigma(m, tau)
        implVol_flat = self.reconstruct_surface(coords, self.BList, scores)
        implVol = implVol_flat.reshape(M.shape)
        
        # Total variance w(m, tau) = sigma(m, tau)^2 * tau = (implVol^2) * T
        w = (implVol**2) * T
    ...
    def compute_price_arbitrage_metrics(self, scores, m_grid=None, tau_grid=None, day_index=0):
        ...
        call_prices_flat = self.implVol_to_price_surface(coords, implVol_flat, day_index=day_index)
    ...
    def measure_raw_arbitrage(self, day_index, tolerance=1e-8):
        ...
        """
        m_clean, tau_clean, implVol_clean = self.cleaned_data[day_index]
        if len(implVol_clean) == 0:
            return {
        ...
        coords = np.column_stack([m_clean, tau_clean])
        call_prices = self.implVol_to_price_surface(coords, implVol_clean, day_index=day_index)
...
if __name__ == '__main__':
    ...
    print(f"DJX Test RMSE (IV): {np.mean(np.sqrt(val_mses_test)):.6f}")
    ...
    print(f"DJX Train RMSE (IV): {np.mean(np.sqrt(val_mses)):.6f}")
    ...
    with open("/Users/macbook/Documents/global_O_Research/O_Research/data/SPX_data/SPX_FPCA_arbPenal.pkl", "wb") as f:
        pickle.dump(fpca.scoreMat, f)
        pickle.dump(fpca.BList, f)
    
    #%% Load Basis representation fit
    fpca = FPCA_penalized_arbPenal(logMoneyness, tau, iv, nb_spline_moneyness = 30, nb_spline_tau = 36, order_moneyness = 4, order_tau = 4, range_moneyness=[-.15,.15])
    with open("/Users/macbook/Documents/global_O_Research/O_Research/data/SPX_data/SPX_FPCA_arbPenal.pkl", "rb") as f:
        fpca.scoreMat = pickle.load(f)
        fpca.BList = pickle.load(f)
    ...
    print(f"SPX Test RMSE (IV): {np.mean(np.sqrt(val_mses_test)):.6f}")
    ...
    print(f"SPX Train RMSE (IV): {np.mean(np.sqrt(val_mses)):.6f}")
