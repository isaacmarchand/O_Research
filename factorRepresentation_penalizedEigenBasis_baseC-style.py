...
class FPCA_penalized:
    def __init__(self, moneyness, tau, iv, nb_spline_moneyness = 10, nb_spline_tau = 10, order_moneyness = 4, order_tau = 4, S=None, r=None, q=None, range_moneyness = [-.15, .1], range_tau = [0, 1]):
        self.S = S
        self.r = r
        self.q = q
    ...
    def first_FPC_fit(self, threshold = 1e-4, maxit = 10, omega_m = 1.0, omega_t = 1.0, d_m = 2, d_t = 2):
        ...
        while j < maxit:
            ...
            Beta = np.linalg.solve(sum_XX + P, sum_Xy)
    ...
    def subsequent_FPC_fit(self, threshold = 1e-4, maxit = 10, omega_m = 1.0, omega_t = 1.0, d_m = 2, d_t = 2):
        ...
        while j < maxit:
            ...
            # [ sum_XX + P  A.T ] [ Beta      ] = [ sum_Xy ]
            # [ A           0   ] [ lambda    ]   [   0    ]
            KKT_A = np.block([
                [sum_XX + P, A.T],
                [A, np.zeros((len(self.BList), len(self.BList)))]
            ])
            KKT_b = np.concatenate([sum_Xy, np.zeros(len(self.BList))])
...
fpca = FPCA_penalized(logMoneyness, tau, iv, nb_spline_moneyness = 10, nb_spline_tau = 12, order_moneyness = 4, order_tau = 4)
with open("/Users/macbook/Documents/global_O_Research/O_Research/data/SPX_data/FPCA_ApproxPenal_logM_tau_iv.pkl", "rb") as f:
...
###################################################################################################
with open("/Users/macbook/Documents/global_O_Research/O_Research/data/DJX_data/DJX_lists.pkl", "rb") as f:
    uniqueDates = pickle.load(f)
    tau = pickle.load(f)
    moneyness = pickle.load(f)
    iv = pickle.load(f)
...
alpha1, B1 = fpca.first_FPC_fit(maxit=20, omega_m=3, omega_t=5)
fpca.plot_eigen_functions(B1, num_points=50, figAngle=-70)

alpha2, B2 = fpca.subsequent_FPC_fit(maxit=30, omega_m=3, omega_t=5)
fpca.plot_eigen_functions(B2, num_points=50, figAngle=-70)

alpha3, B3 = fpca.subsequent_FPC_fit(maxit=30, omega_m=3, omega_t=5)
fpca.plot_eigen_functions(B3, num_points=50, figAngle=-70)
...
with open("/Users/macbook/Documents/global_O_Research/O_Research/data/DJX_data/DJX_FPCA_ApproxPenal_logM_tau_iv.pkl", "wb") as f:
    pickle.dump(fpca.scoreMat, f)
    pickle.dump(fpca.BList, f)
...
with open("/Users/macbook/Documents/global_O_Research/O_Research/data/DJX_data/DJX_FPCA_ApproxPenal_logM_tau_iv.pkl", "rb") as f:
    fpca.scoreMat = pickle.load(f)
    fpca.BList = pickle.load(f)
